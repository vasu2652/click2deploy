...
const configGenerator = async () => {
  ...
  let questions = Object.keys(questionsJSON).reduce((agg, key) => {
    let when = true
    if (questionsJSON[key].depends_on && questionsJSON[key].depends_on.length > 0) {
      when = answers => !questionsJSON[key].depends_on.map(ele => answers[ele]).includes(false)
    }
    return [
      ...agg,
      {
...
class DeployCommand extends Command {
  ...
  async deploy(config, bashFilePath) {
    ...
    shipit.blTask('restart', async () => {
      await shipit.copyToRemote(config.default.localEnvFilePath, shipit.releasePath)
...
DeployCommand.flags = {
  config: flags.string({
    char: 'c',
    description: 'Absolute Path of the Config File',
  }),
  filename: flags.string({
    char: 'f',
    description: 'Config file name which you have created before',
  }),
}
